...
double truncated_Poisson_coupling(int k, const double average) {
    ...
    if(average<=0 || k<=0 || isinf(average) || isnan(average)) return 0.0;
        ...
        if(isinf(answer) || answer>1.0) answer = 1.0;
...
// ODE system for diffusion
    ...
    for(int d01=0;d01<p.N;++d01) for(int d02=0;d02<p.N;++d02) { //loop over sites
        ...
        for(int d4=0;d4<p.MF4-1;++d4) { //loop over recovered individuals
            for(int d3=0;d3<p.MF3-2;++d3) { //loop over infectious individuals
                //Loop over diffusion kernel (currently von neumann neighborhood)
                if(d01>0) diffusion_matrix[d01][d02] += diff_rate*d3*yref[d01-1][d02][0][d3][d4];
                if(d01<p.N-1) diffusion_matrix[d01][d02] += diff_rate*d3*yref[d01+1][d02][0][d3][d4];
                if(d02>0) diffusion_matrix[d01][d02] += diff_rate*d3*yref[d01][d02-1][0][d3][d4];
                if(d02<p.N-1) diffusion_matrix[d01][d02] += diff_rate*d3*yref[d01][d02+1][0][d3][d4];
            }
            //Diffusion from mean-field limit of infectious individuals
            //Loop over diffusion kernel (currently von neumann neighborhood)
            if(d01>0) diffusion_matrix[d01][d02] += diff_rate*yref[d01-1][d02][0][p.MF3-1][d4]*yref[d01-1][d02][0][p.MF3-2][d4];
            if(d01<p.N-1) diffusion_matrix[d01][d02] += diff_rate*yref[d01+1][d02][0][p.MF3-1][d4]*yref[d01+1][d02][0][p.MF3-2][d4];
            if(d02>0) diffusion_matrix[d01][d02] += diff_rate*yref[d01][d02-1][0][p.MF3-1][d4]*yref[d01][d02-1][0][p.MF3-2][d4];
            if(d02<p.N-1) diffusion_matrix[d01][d02] += diff_rate*yref[d01][d02+1][0][p.MF3-1][d4]*yref[d01][d02+1][0][p.MF3-2][d4];
        }
    }
        ...
        for(int d3=0; d3<p.MF3-1; ++d3) {
          for(int d4=0; d4<p.MF4-1; ++d4) {
            ...
            if(d3<p.MF3-3 && d4<p.MF4-3) { //general master equation
                ...
                if(d3>0) fref[d01][d02][0][d3][d4] += (p.beta*(d3-1)+diffusion_matrix[d01][d02])*(S+1)*yref[d01][d02][0][d3-1][d4];
                if(d4>0) fref[d01][d02][0][d3][d4] += 1.0*(d3+1)*p.alpha*yref[d01][d02][0][d3+1][d4-1];
            }

            if(d3==p.MF3-3 && d4<p.MF4-3) { //final states of master equation for infectious
                double S = Population(t,d01,d02)-d3-d4;
                fref[d01][d02][0][d3][d4] = -(p.beta*d3+diffusion_matrix[d01][d02])*S*yref[d01][d02][0][d3][d4] - 1.0*d3*p.alpha*yref[d01][d02][0][d3][d4];
                fref[d01][d02][0][d3][d4] += p.gamma*(d4+1)*yref[d01][d02][0][d3][d4+1] - p.gamma*d4*yref[d01][d02][0][d3][d4];
                if(d3>0) fref[d01][d02][0][d3][d4] += (p.beta*(d3-1)+diffusion_matrix[d01][d02])*(S+1)*yref[d01][d02][0][d3-1][d4];
                if(d4>0) fref[d01][d02][0][d3][d4] += truncated_Poisson_coupling(d3+1, yref[d01][d02][0][d3+2][d4-1])*(d3+1)*p.alpha*yref[d01][d02][0][d3+1][d4-1];
            }

            if(d3<p.MF3-3 && d4==p.MF4-3) { //final states of master equation for recovered
                double S = Population(t,d01,d02)-d3-d4;
                fref[d01][d02][0][d3][d4] = -(p.beta*d3+diffusion_matrix[d01][d02])*S*yref[d01][d02][0][d3][d4] - 1.0*d3*p.alpha*yref[d01][d02][0][d3][d4];
            ...
            if(d3==p.MF3-3 && d4==p.MF4-3) { //final states of master equations for infectious + recovered
                double S = Population(t,d01,d02)-d3-d4;
                fref[d01][d02][0][d3][d4] = -(p.beta*d3+diffusion_matrix[d01][d02])*S*yref[d01][d02][0][d3][d4] - 1.0*d3*p.alpha*yref[d01][d02][0][d3][d4];
                fref[d01][d02][0][d3][d4] += p.gamma*(d4+1)*yref[d01][d02][0][d3][d4+1]*truncated_Poisson_coupling(d4+1, yref[d01][d02][0][d3][d4+2]) - p.gamma*d4*yref[d01][d02][0][d3][d4];
                if(d3>0) fref[d01][d02][0][d3][d4] += (p.beta*(d3-1)+diffusion_matrix[d01][d02])*(S+1)*yref[d01][d02][0][d3-1][d4];
            ...
            if(d3==p.MF3-2 && d4<p.MF4-3) {//infectious mean-field equation, general recovered master equation
                double S = Population(t,d01,d02)-yref[d01][d02][0][d3+1][d4]-d4;
                if(d3>0) fref[d01][d02][0][d3][d4] = (p.beta*(d3-1)+diffusion_matrix[d01][d02])*(Population(t,d01,d02)-d3+1-d4)*yref[d01][d02][0][d3-1][d4];
                fref[d01][d02][0][d3][d4] += p.gamma*(d4+1)*yref[d01][d02][0][d3][d4+1] - p.gamma*d4*yref[d01][d02][0][d3][d4];
                ...
                if(d4>0) fref[d01][d02][0][d3][d4] += (1.0-truncated_Poisson_coupling(d3, yref[d01][d02][0][d3+1][d4-1]))*yref[d01][d02][0][d3+1][d4-1]*p.alpha*yref[d01][d02][0][d3][d4-1];
            ...
            if(d3==p.MF3-2 && d4==p.MF4-3) {//infectious mean-field equation, final recovered master equation state
                double S = Population(t,d01,d02)-yref[d01][d02][0][d3+1][d4]-d4;
                if(d3>0) fref[d01][d02][0][d3][d4] = (p.beta*(d3-1)+diffusion_matrix[d01][d02])*(Population(t,d01,d02)-d3+1-d4)*yref[d01][d02][0][d3-1][d4];
                fref[d01][d02][0][d3][d4] += p.gamma*(d4+1)*yref[d01][d02][0][d3][d4+1]*truncated_Poisson_coupling(d4+1, yref[d01][d02][0][d3][d4+2]) - p.gamma*d4*yref[d01][d02][0][d3][d4];
                fref[d01][d02][0][d3][d4] -= p.alpha*yref[d01][d02][0][d3+1][d4]*(1.0-truncated_Poisson_coupling(d3, yref[d01][d02][0][d3+1][d4]))*yref[d01][d02][0][d3][d4];
            ...
            if(d3<p.MF3-3 && d4==p.MF4-2) { //general infectious master equation, recovered mean-field equation
                ...
                if(d3>0) fref[d01][d02][0][d3][d4] += (p.beta*(d3-1)+diffusion_matrix[d01][d02])*(Population(t,d01,d02)-d3+1-yref[d01][d02][0][d3-1][d4+1])*yref[d01][d02][0][d3-1][d4];
                if(d4>0) fref[d01][d02][0][d3][d4] += 1.0*(d3+1)*p.alpha*yref[d01][d02][0][d3+1][d4-1];
                ...
                if(d4>0) fref[d01][d02][0][d3][d4+1] +=1.0*(d3+1)*p.alpha*yref[d01][d02][0][d3+1][d4-1];
                fref[d01][d02][0][d3][d4+1] -= p.gamma*yref[d01][d02][0][d3][d4+1]*yref[d01][d02][0][d3][d4];  //mean-field state
            }

            if(d3==p.MF3-3 && d4==p.MF4-2) { // final infectious master equation state, recovered mean-field equation
                double S = Population(t,d01,d02)-d3-yref[d01][d02][0][d3][d4+1];
                fref[d01][d02][0][d3][d4] = -(p.beta*d3+diffusion_matrix[d01][d02])*S*yref[d01][d02][0][d3][d4] - d3*p.alpha*yref[d01][d02][0][d3][d4];
                fref[d01][d02][0][d3][d4] -= p.gamma*d4*yref[d01][d02][0][d3][d4]*truncated_Poisson_coupling(d4, yref[d01][d02][0][d3][d4+1]);
                if(d3>0) fref[d01][d02][0][d3][d4] += (p.beta*(d3-1)+diffusion_matrix[d01][d02])*(Population(t,d01,d02)-d3+1-yref[d01][d02][0][d3-1][d4+1])*yref[d01][d02][0][d3-1][d4];
                if(d4>0) fref[d01][d02][0][d3][d4] += 1.0*(d3+1)*truncated_Poisson_coupling(d3+1, yref[d01][d02][0][d3+2][d4-1])*p.alpha*yref[d01][d02][0][d3+1][d4-1];
                ...
                if(d4>0) fref[d01][d02][0][d3][d4+1] += 1.0*(d3+1)*truncated_Poisson_coupling(d3+1, yref[d01][d02][0][d3+2][d4-1])*p.alpha*yref[d01][d02][0][d3+1][d4-1];
                fref[d01][d02][0][d3][d4+1] -= p.gamma*yref[d01][d02][0][d3][d4+1]*yref[d01][d02][0][d3][d4];  //mean-field state
            }

            if(d3==p.MF3-2 && d4==p.MF4-2) { // double mean-regime
                double S = Population(t,d01,d02)-yref[d01][d02][0][d3+1][d4]-yref[d01][d02][0][d3][d4+1];
                if(d3>0) fref[d01][d02][0][d3][d4] = -1.0*d3*p.alpha*yref[d01][d02][0][d3][d4]*truncated_Poisson_coupling(d3, yref[d01][d02][0][d3+1][d4]);
                if(d4>0) fref[d01][d02][0][d3][d4] -= p.gamma*d4*yref[d01][d02][0][d3][d4]*truncated_Poisson_coupling(d4, yref[d01][d02][0][d3][d4+1]);
                if(d3>0) fref[d01][d02][0][d3][d4] += (p.beta*(d3-1)+diffusion_matrix[d01][d02])*(Population(t,d01,d02)-d3+1-yref[d01][d02][0][d3-1][d4+1])*yref[d01][d02][0][d3-1][d4];
                if(d4>0) fref[d01][d02][0][d3][d4] += (1.0-truncated_Poisson_coupling(d3, yref[d01][d02][0][d3+1][d4-1]))*yref[d01][d02][0][d3+1][d4-1]*p.alpha*yref[d01][d02][0][d3][d4-1];

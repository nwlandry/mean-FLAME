...
int dydt(double t, const double y[], double f[], void *param)
{
    ...
    for (int d1 = 0; d1 < p.N; ++d1)
        ...
        { // loop over sites
            ...
            { // loop over sappling numbers
                ...
                // Loop over diffusion kernel (currently von neumann neighborhood)
                if (d1 > 0)
                    diffusion_matrix[d1][d2] += diff_rate * yref[d1 - 1][d2][0][d3][p.MF4 - 1] * yref[d1 - 1][d2][0][d3][p.MF4 - 2];
                if (d1 < p.N - 1)
                    diffusion_matrix[d1][d2] += diff_rate * yref[d1 + 1][d2][0][d3][p.MF4 - 1] * yref[d1 + 1][d2][0][d3][p.MF4 - 2];
                if (d2 > 0)
                    diffusion_matrix[d1][d2] += diff_rate * yref[d1][d2 - 1][0][d3][p.MF4 - 1] * yref[d1][d2 - 1][0][d3][p.MF4 - 2];
                if (d2 < p.N - 1)
                    diffusion_matrix[d1][d2] += diff_rate * yref[d1][d2 + 1][0][d3][p.MF4 - 1] * yref[d1][d2 + 1][0][d3][p.MF4 - 2];
            }
        }
    ...
    for (int d1 = 0; d1 < p.N; ++d1)
    {
        ...
        { // loop over sites
            ...
            for (int d3 = 0; d3 < p.MF3 - 1; ++d3)
            {
                ...
                for (int d4 = 0; d4 < p.MF4 - 2; ++d4)
                {
                    fref[d1][d2][1][d3][d4] = diffusion_matrix[d1][d2] + diff_rate * d4 * yref[d1][d2][0][d3][d4] - yref[d1][d2][1][d3][d4] * (p.seeddeath + p.seed2seedling);
                    fref[d1][d2][2][d3][d4] = yref[d1][d2][1][d3][d4] * p.seed2seedling - yref[d1][d2][2][d3][d4] * (p.seedlingdeath + p.seedling2sapling);
                }
                // Plus dynamics with mean-field adult trees
                fref[d1][d2][1][d3][p.MF4 - 2] = diffusion_matrix[d1][d2] + diff_rate * yref[d1][d2][0][d3][p.MF4 - 1] * yref[d1][d2][0][d3][p.MF4 - 2] - yref[d1][d2][1][d3][p.MF4 - 2] * (p.seeddeath + p.seed2seedling);
                fref[d1][d2][2][d3][p.MF4 - 2] = yref[d1][d2][1][d3][p.MF4 - 2] * p.seed2seedling - yref[d1][d2][2][d3][p.MF4 - 2] * (p.seedlingdeath + p.seedling2sapling);
            }
        }
    }
    ...
    for (int d1 = 0; d1 < p.N; ++d1)
    {
        for (int d2 = 0; d2 < p.N; ++d2)
        {
            ...
            double K = CarryingCapacity(t, d2, d1);
            ...
            for (int d3 = 0; d3 < p.MF3 - 1; ++d3)
            {
                for (int d4 = 0; d4 < p.MF4 - 1; ++d4)
                {
                    ...
                    { // general master equation
                        ...
                        fref[d1][d2][0][d3][d4] = yref[d1][d2][0][d3 + 1][d4] * (d3 + 1) * p.sapplingdeath; // sapling death influx
                        if (d3 > 0)
                        ...
                        if (d3 > 0)
                            fref[d1][d2][0][d3][d4] -= yref[d1][d2][0][d3][d4] * d3 * p.sapplingdeath + yref[d1][d2][0][d3][d4] * p.sapling2tree * pow(1.0 * d3, 0.5) * Growth(1.0 * d4, K); // sapling loss outflux
                        if (d4 > 0)
                            fref[d1][d2][0][d3][d4] += yref[d1][d2][0][d3 + 1][d4 - 1] * p.sapling2tree * pow(1.0 + d3, 0.5) * Growth(1.0 * d4 - 1.0, K); // sapling growth influx
                        fref[d1][d2][0][d3][d4] -= yref[d1][d2][0][d3][d4] * d4 * p.treedeath;                                                            // tree loss outflux
                        fref[d1][d2][0][d3][d4] += yref[d1][d2][0][d3][d4 + 1] * (d4 + 1) * p.treedeath;                                                  // tree loss influx
                    }
                    ...
                    { // final states of master equation for sappling
                        ...
                        if (d3 > 0)
                            fref[d1][d2][0][d3][d4] -= yref[d1][d2][0][d3][d4] * d3 * p.sapplingdeath + yref[d1][d2][0][d3][d4] * p.sapling2tree * pow(1.0 * d3, 0.5) * Growth(1.0 * d4, K); // sapling loss outflux
                        fref[d1][d2][0][d3][d4] += yref[d1][d2][0][d3 + 1][d4] * (d3 + 1) * p.sapplingdeath * Poisson_coupling(d3 + 1, yref[d1][d2][0][d3 + 2][d4]);                         // sapling death influx
                        if (d4 > 0)
                    ...
                    { // final states of master equation for trees
                        ...
                        if (d4 > 0)
                            fref[d1][d2][0][d3][d4] += yref[d1][d2][0][d3 + 1][d4 - 1] * p.sapling2tree * pow(1.0 + d3, 0.5) * Growth(1.0 * d4 - 1.0, K);          // sapling growth influx
                        fref[d1][d2][0][d3][d4] -= yref[d1][d2][0][d3][d4] * d4 * p.treedeath;                                                                     // tree loss outflux
                        fref[d1][d2][0][d3][d4] += yref[d1][d2][0][d3][d4 + 1] * (d4 + 1) * p.treedeath * Poisson_coupling(d4 + 1, yref[d1][d2][0][d3][d4 + 2]); // tree loss influx
                    }
                    ...
                    { // sapplings mean-field equation, general adult master equation
                        ...
                        fref[d1][d2][0][d3][d4] -= yref[d1][d2][0][d3][d4] * d3 * p.sapplingdeath * Poisson_coupling(d3, yref[d1][d2][0][d3 + 1][d4]); // occupation number outflux
                        if (d3 > 0)
                        ...
                        if (yref[d1][d2][0][d3 + 1][d4] > 0)
                            fref[d1][d2][0][d3 + 1][d4] = yref[d1][d2][2][d3][d4] * p.seedling2sapling - yref[d1][d2][0][d3 + 1][d4] * p.sapplingdeath + p.sapling2tree * pow(yref[d1][d2][0][d3 + 1][d4], 0.5) * Growth(1.0 * d4, K); // mean-field state
                        else
                            fref[d1][d2][0][d3 + 1][d4] = yref[d1][d2][2][d3][d4] * p.seedling2sapling - yref[d1][d2][0][d3 + 1][d4] * (p.sapplingdeath); // mean-field state
                    }
                    ...
                    { // sapplings mean-field equation, final adult master equation state
                        ...
                        if (d3 > 0)
                            fref[d1][d2][0][d3][d4] = yref[d1][d2][2][d3 - 1][d4] * p.seedling2sapling * yref[d1][d2][0][d3 - 1][d4];                  // occupation number influx
                        fref[d1][d2][0][d3][d4] -= yref[d1][d2][0][d3][d4] * d3 * p.sapplingdeath * Poisson_coupling(d3, yref[d1][d2][0][d3 + 1][d4]); // occupation number outflux (diagonal move)
                        if (d3 > 0)
                            fref[d1][d2][0][d3][d4] -= yref[d1][d2][0][d3][d4] * p.sapling2tree * pow(1.0 * d3, 0.5) * Growth(1.0 * d4, K) * Poisson_coupling(d3, yref[d1][d2][0][d3 + 1][d4]); // occupation number outflux (diagonal move)
                        if (yref[d1][d2][0][d3 + 1][d4] > 0)
                            fref[d1][d2][0][d3][d4] -= yref[d1][d2][0][d3][d4] * yref[d1][d2][0][d3 + 1][d4] * p.sapling2tree * pow(yref[d1][d2][0][d3 + 1][d4], -0.5) * Growth(1.0 * d4, K) * (1 - Poisson_coupling(d3, yref[d1][d2][0][d3 + 1][d4])); // occupation number outflux (lateral move)
                    ...
                    { // general sapplings master equation, adult tree mean-field equation
                        ...
                        fref[d1][d2][0][d3][d4] -= yref[d1][d2][0][d3][d4] * d4 * p.treedeath * Poisson_coupling(d4, yref[d1][d2][0][d3][d4 + 1]);     // occupation number outflux
                        if (d3 > 0)
                            fref[d1][d2][0][d3][d4] += p.seedling2sapling * yref[d1][d2][2][d3 - 1][d4] * yref[d1][d2][0][d3 - 1][d4];
                        fref[d1][d2][0][d3][d4] -= p.seedling2sapling * yref[d1][d2][2][d3][d4] * yref[d1][d2][0][d3][d4]; // seedling to sapling mean-field forcing
                        if (d3 > 0)
                            fref[d1][d2][0][d3][d4] -= yref[d1][d2][0][d3][d4] * d3 * p.sapplingdeath + yref[d1][d2][0][d3][d4] * p.sapling2tree * pow(1.0 * d3, 0.5) * Growth(yref[d1][d2][0][d3][d4 + 1], K); // sapling loss outflux
                        ...
                        if (d3 > 0)
                            fref[d1][d2][0][d3][d4 + 1] = 1.0 * pow(1.0 * d3, 0.5) * p.sapling2tree * Growth(yref[d1][d2][0][d3][d4 + 1], K) - yref[d1][d2][0][d3][d4 + 1] * p.treedeath; // mean-field state
                        else
                            fref[d1][d2][0][d3][d4 + 1] = -yref[d1][d2][0][d3][d4 + 1] * p.treedeath; // mean-field state
                    }
                    ...
                    { // double mean-regime
                        ...
                        if (d3 > 0)
                            fref[d1][d2][0][d3][d4] -= (yref[d1][d2][0][d3][d4] * d3 * p.sapplingdeath + yref[d1][d2][0][d3][d4] * p.sapling2tree * pow(1.0 * d3, 0.5) * Growth(yref[d1][d2][0][d3][d4 + 1], K)) * Poisson_coupling(d3, yref[d1][d2][0][d3 + 1][d4]); // sappling MF occupation number outflux
                        if (d4 > 0 && yref[d1][d2][0][d3 + 1][d4 - 1] > 0)
                        ...
                        fref[d1][d2][0][d3][d4] -= yref[d1][d2][0][d3][d4] * d4 * p.treedeath * Poisson_coupling(d4, yref[d1][d2][0][d3][d4 + 1]);                                                                                           // tree MF occupation number outflux

                        if (yref[d1][d2][0][d3 + 1][d4] > 0)
                            fref[d1][d2][0][d3 + 1][d4] = yref[d1][d2][2][d3][d4] * p.seedling2sapling - yref[d1][d2][0][d3 + 1][d4] * p.sapplingdeath - p.sapling2tree * pow(yref[d1][d2][0][d3 + 1][d4], 0.5) * Growth(yref[d1][d2][0][d3][d4 + 1], K); // sappling mean-field state
                        else
                            fref[d1][d2][0][d3 + 1][d4] = yref[d1][d2][2][d3][d4] * p.seedling2sapling - yref[d1][d2][0][d3 + 1][d4] * (p.sapplingdeath); // sappling mean-field state
                        if (yref[d1][d2][0][d3 + 1][d4] > 0)
                            fref[d1][d2][0][d3][d4 + 1] = pow(yref[d1][d2][0][d3 + 1][d4], 0.5) * p.sapling2tree * Growth(yref[d1][d2][0][d3][d4 + 1], K) - yref[d1][d2][0][d3][d4 + 1] * p.treedeath; // tree mean-field state
                        else
                            fref[d1][d2][0][d3][d4 + 1] = -yref[d1][d2][0][d3][d4 + 1] * p.treedeath; // tree mean-field state
                    }
                }
            }
        }
